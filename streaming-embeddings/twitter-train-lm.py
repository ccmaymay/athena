...
def train_lm(ns, conn):
    ...
    try:
        ...
        while d is not None:
            if d['message_type'] == 'ping':
                ...
                model.sentence_increment(tokens)
    ...
    logging.info(u'truncating LM to %d words ...' % ns.vocab_dim)
    model.truncate_language_model(ns.vocab_dim)

    logging.info(u'resetting negative-sampling strategy according to LM ...')
    model.reset_neg_sampling_strategy(
        ns.smoothing_exponent,
        ns.smoothing_offset)
...
def main():
    ...
    parser = ArgumentParser(
        'train SpaceSaving word2vec language model (do not update embeddings)',
    ...
    (train_lm_conn, child_conn) = Pipe()
    lm_process = Process(target=train_lm, args=(ns, child_conn))
    lm_process.start()
    ...
    try:
        for d in extract_redis_tweet_text(host, port, key,
            ...
            if not lm_process.is_alive():
                logging.warning('lm child process not alive')
                break
            ...
            train_lm_conn.send(d)

            train_lm_conn.send(dict(message_type='ping'))
            if train_lm_conn.recv().get('message_type') != 'pong':
                logging.warning('lm child process did not pong to ping')
                break

        logging.info('signaling children to exit')

        train_lm_conn.send(None)
